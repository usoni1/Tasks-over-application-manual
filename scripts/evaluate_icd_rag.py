...
DEFAULT_MLFLOW_EXPERIMENT = "icd-rag-evaluate"
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the ICD RAG baseline with mlflow.evaluate.")
...
def resolve_model_name() -> str:
    ...
    return first_non_empty(os.environ.get("ICD_MODEL_NAME"), os.environ.get("OPENAI_CHAT_MODEL"), os.environ.get("MODEL_NAME"), DEFAULT_MODEL_NAME) or DEFAULT_MODEL_NAME
...
def main(argv: list[str] | None = None) -> int:
    ...
    from baselines.icd_rag.single_case import fetch_case_record, run_single_case_prediction, score_prediction

    config = load_config(args)
    execution_env = args.execution_env or os.environ.get("EXECUTION_ENV", "local")
    spark = resolve_spark_session(execution_env=execution_env, app_name="icd-rag-evaluate")
    prompts_dir = PROJECT_ROOT / "baselines" / "icd_rag" / "prompts"
    model_name = args.model_name or resolve_model_name()
    run_name = args.mlflow_run_name or f"icd_rag_eval_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
    ...
    with mlflow.start_run(run_name=run_name) as run:
        ...
        progress_bar = tqdm(total=len(eval_df), desc="ICD eval cases", unit="case")
        progress_logger = EvaluationProgressLogger(logger=logger, label="ICD eval", total=len(eval_df))
            ...
            try:
                with mlflow.start_span(name=f"hadm_{hadm_id}") as case_span:
                    ...
                    try:
                        ...
                        logger.exception("ICD evaluation failed for hadm_id=%s note_id=%s", hadm_id, note_id)
        ...
        mlflow.log_dict(summary, "evaluation/icd_rag_summary.json")
        mlflow.log_dict(to_jsonable({"rows": evaluation_df.to_dict(orient="records")}), "evaluation/icd_rag_rows.json")
        if "eval_results" in evaluation.tables:
            mlflow.log_dict(to_jsonable({"rows": evaluation.tables["eval_results"].to_dict(orient="records")}), "evaluation/icd_rag_genai_eval.json")
